...
封装所有模型脚本的重复代码：数据加载、训练函数、评估循环、可视化。
各 risk_modeling_*.py 只需配置 + 串流程，不再各自维护 ~500 行。
"""
import matplotlib
matplotlib.use('Agg')
...
def get_feature_cols(df):
    exclude = ['vehicle_id', 'location', 'side', 'source', 'risk']
    ...
    for loc_key in loc_keys:
        for side in ['left', 'right']:
            ...
            for (vid, src), grp in df.groupby(['ID', 'Source']):
                ...
                row['risk'] = overall_risk(grp, v0_kmh=v0)
                all_v.append(row)
    ...
    for loc_key in loc_keys:
        for side in ['left', 'right']:
            ...
            for (vid, src), grp in df.groupby(['ID', 'Source']):
                ...
                v0 = v0_for_loc(loc_key, loc_keys, default_v0, loc5_v0)
                risk = overall_risk(grp, v0_kmh=v0)

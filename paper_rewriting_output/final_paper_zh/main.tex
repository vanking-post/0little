...
EWM 权重仅依赖 5 项 SSM 指标的风险激活值矩阵 $\mathbf{X}$，完全不访问模型训练所用的 60 维样本特征和风险标签，保证了权重标定的统计独立性和可复现性。由上述流程得出的变道车辆权重为 $\{w_j\}^{5}_{j=1} = \{0.247, 0.083, 0.131, 0.356, 0.183\}$（mTTC/THW/PET/F\_ETTC\newline/OL\_PET），跟驰车辆权重为 $\{0.463, 0.114, 0.423\}$（mTTC/THW/B\_mTTC）。值得关注的是 F\_ETTC 获得了最高权重（0.356），显著高于其经验估计值（0.20），表明前向扩展碰撞时间在变道车辆间具有最大的判别力差异——即不同变道事件中前向碰撞风险的异质性远超其他四个指标；而 THW 仅获 0.083，说明车头时距的分布在车辆间高度集中（$E_j$ 接近 1），单独权重较小。
...
\textbf{(5) 原车道后车侵入时间（OL\_PET）。} 针对相邻车道变道场景，度量变道车辆（SV）跨越车道线的时刻与原车道后车（FV）到达同一变道点的时间差：
...
式(8)为逐帧风险分，式(9)为单辆车最终风险分（取各指标全窗口最危险帧的最大贡献再求和）。$\mathcal{M} = \{\text{mTTC}, \text{THW}, \text{PET}, \text{F\_ETTC}, \text{OL\_PET}\}$，EWM 客观权重 $\{w_i\} = \{0.247, 0.083, 0.131, 0.356, 0.183\}$。$K$ 为下节定义的速度修正因子。

\subsubsection{替代安全指标的实测分布特征}

为进一步验证五项 SSM 指标的统计分布特性及其对应的风险判别能力，对全量 1,574 辆变道车辆的 118,050 帧轨迹数据（每车 75 帧）进行概率分布拟合与危险区间占比统计。图~\ref{fig:ssm_distributions} 展示了 THW、PET、OL\_PET、mTTC 和 F\_ETTC 五项指标的实测分布直方图及最优拟合曲线。各指标均标注了 2 s 危险阈值（红色虚线）、5 s 谨慎阈值（黄色虚线）及均值线。

\begin{figure}[H]
\centering
\begin{subfigure}{0.32\textwidth}
\centering\includegraphics[width=\textwidth]{dist_F_ETTC_hist.png}
\caption{F\_ETTC}
\end{subfigure}
\hfill
\begin{subfigure}{0.32\textwidth}
\centering\includegraphics[width=\textwidth]{dist_PET.png}
\caption{PET}
\end{subfigure}
\hfill
\begin{subfigure}{0.32\textwidth}
\centering\includegraphics[width=\textwidth]{dist_OL_PET.png}
\caption{OL\_PET}
\end{subfigure}
...
\centering\includegraphics[width=\textwidth]{dist_mTTC.png}
\caption{mTTC}
\end{subfigure}
\hfill
\begin{subfigure}{0.48\textwidth}
\centering\includegraphics[width=\textwidth]{dist_Time_Headway.png}
\caption{THW}
\end{subfigure}
\caption{五项替代安全指标的实测帧级分布。直方图为实测值，深灰实线为最优拟合曲线，浅灰虚线为备选拟合；红色虚线为 2 s 危险阈值，黄色虚线为 5 s 谨慎阈值。}\label{fig:ssm_distributions}
\end{figure}

五项指标均呈现典型的右偏分布，符合交通冲突数据"多数样本安全、少数样本危险"的普遍规律。其中四项以对数正态分布为最优拟合，与交通流参数（车头时距、冲突时间）的经典统计特征一致\cite{C18}。从危险区间占比（$<$2 s）可区分两类风险等级：（1）\textbf{紧迫型}——THW（中位数 1.61 s），超六成帧处于 2 s 危险线以内，表明变道过程中跟车行为整体偏紧，是变道风险最核心的驱动指标；（2）\textbf{中低紧迫型}——PET（中位数 2.08 s）、OL\_PET（中位数 2.88 s）、mTTC（非零中位数 9.00 s）和 F\_ETTC（非零中位数 27.14 s）。其中 PET 中位数紧贴危险阈值，对冲突紧迫性具有次高灵敏度；OL\_PET 反映换道越线对原车道后车的时间压迫程度中等；mTTC 和 F\_ETTC 的危险区间占比极低——mTTC 因加速度修正项在减速避险场景中可显著增大碰撞时间估值，削弱分布尾部极端值聚集，是五项指标中唯一以伽马分布为最优拟合的指标；F\_ETTC 的非零帧占比仅 26.8\%，其宽幅分布表明将碰撞时间拓展至二维平面后，综合危险方向的逼近速率整体低于纯纵向情形。

风险紧迫性排序 $\text{THW} > \text{PET} > \text{OL\_PET} \gg \text{F\_ETTC} > \text{mTTC}$ 表明了不同指标的判别力差异。这一分布特征直接影响了后续衰减常数 $k_i$ 的取值策略：对紧迫型指标（THW）需在常见危险区间（0$\sim$5 s）内保持足够分辨率，故取 $k=6$ s；对中低紧迫型指标（PET、OL\_PET、mTTC、F\_ETTC）则以更大的 $k$ 值拓展有效响应区间。
...
SMOTE 合成样本依赖训练路段内部结构，无法表征未见路段风险特征，因此跨路段中不升反降；所有 SMOTE 实验均在每个 fold 训练集内部独立执行，杜绝了数据泄露。Optuna 贝叶斯优化在树模型上取得正向效果（XGBoost +0.008, RF +0.020），但 LSTM 仍处低位。ST 联合取得最优跨路段 F1（0.764），较单独 Optuna 仅提升微小，说明样本合成与超参数搜索在跨路段泛化上不具有正向叠加。PCA 降维（15$\to$12 维）后 LSTM 的 F1 从 0.598 降至 0.509（图~\ref{fig:pca_variance}），进一步印证了时序相关模式承载关键判别信息，降维反而放大了路段间性能鸿沟。
...
LSTM & 0.490$\pm$0.124 & 0.476$\pm$0.125 & 0.444$\pm$0.118 \\
\bottomrule
\end{tabular}
\end{table}
...
在随机 80/20 划分（同分布内评估）中（表~\ref{tab:random}），XGBoost 的 F1 升至 0.793，RF 升至 0.763，MLP 升至 0.659，LSTM 升至 0.602。XGBoost 跨路段退化幅度约 2.9 个百分点，表明其特征-风险映射具有较好的跨路段可迁移性。
...
树集成模型在跨路段泛化中显著优于神经网络的核心原因在于模型归纳偏置差异。树模型的轴对齐划分在面对特征分布漂移时具有分段常数鲁棒性——只要测试样本落在训练集叶节点取值范围内即可合理分类。MLP 的逐层非线性映射将训练集协方差结构全面编码至权重矩阵，一旦测试集协方差因路段差异改变便产生剧烈退化（退化 4.9\% vs XGBoost 的 1.7\%）。LSTM 的劣势源于双重约束：样本量不足以支持深度循环网络充分训练，且原始时序输入的跨路段分布漂移远超聚合特征——后者经人工设计后具有物理含义的路段间不变性（如"均值较大者风险较低"的单调关系保持），而不同路段变道时序动态模式存在系统性差异（如 Loc5 的加速积累短于 Loc1，Loc3 驾驶人更早开始横向偏移）。
...
（1）树集成模型显著优于神经网络模型。XGBoost 最优（Weighted F1=0.754$\pm$0.069），跨路段退化仅 1.4\%，具备跨路段部署的现实可行性；MLP 退化高达 4.9\%。

（2）路段几何形态是关键调节变量。路基段间迁移性良好（F1=0.796$\sim$0.821），分合流区泛化显著下降（F1=0.631），需部署专用模型。
...
\appendix
\section{附注}
实验配置：XGBoost：500棵基学习器，最大树深5，学习率0.05，列采样率0.8，早停轮次20，多类softmax目标函数。RF：300棵决策树，最大树深10，最小叶节点样本数4，class\_weight=`balanced'。MLP：两层隐藏层（64$\to$32神经元），ReLU激活，自适应学习率，早停正则化，StandardScaler标准化。LSTM：两层循环网络（64$\to$32神经元），25\% Dropout，Adam优化器（初始学习率$5\times10^{-4}$），早停耐心值15。

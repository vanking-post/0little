...
# TTC阈值参数
TTC_LOW_THRESHOLD = 100.5  # TTC下限阈值
...
print(f"{'=' * 50}")
print(f"数据处理开始: 交通流数据补全与TTC重新计算（增强版 - TTC统计对比）")
...
if len(special_ttc_changes) > 0:
    ...
    for idx, row in special_ttc_changes.head(SPECIAL_TTC_PRINT_ROWS).iterrows():
        ...
        # 获取前车速度
        front_data = traffic_flows_west_complete_with_original[
            (traffic_flows_west_complete_with_original['ID'] == front_id) &
            (traffic_flows_west_complete_with_original['Frame'] == row['Frame'])
            ]

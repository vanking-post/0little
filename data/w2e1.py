...
if not os.path.exists(file_path):
    ...
    for vehicle_id, group in df_sorted.groupby('ID'):
        ...
        if len(unique_seq) < 2:
            ...
            # 如果是匝道车(8)且没变道，直接跳过（因为它没进入主路，不符合你的研究需求）
            continue
        ...
        if unique_seq[0] == 8:
            ...
            if category:
                ...
                # 采样逻辑：匝道变道（8->7, 7->6等均为左变道）
                # 检查第一变道点是否满足阈值且具备采样空间
                first_idx = change_idx[0]
                if first_idx >= THRESHOLD_RAMP_FIRST_CHANGE and first_idx >= 100:
                    sample = group.iloc[first_idx - 100: first_idx - 50].copy()
                    sample['Label'] = '左变道'
                    sampling_data.append(sample)
                    source_sample_counts['左变道']['匝道驶入变道'] += 1

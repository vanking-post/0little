#进行数据清洗，将不合理的数据进行删除
#长宽不合理的，碰撞时间重新核算，速度、加速度以及坐标的处理，矢量单调性的处理
#危险指标如车头时距、跟车距离以及碰撞时间的剔除与核算
#读取traffic_flows_west/east,输出traffic_flows_west/east_processed
import pandas as pd
import numpy as np
import gc
import os
...
traffic_flows_west = pd.read_pickle(east_pkl_path)
trace_csv_path = os.path.join(save_dir, "traffic_flows_east_modify_trace.csv")
processed_path = os.path.join(save_dir, "traffic_flows_east_processed.pkl")
processed_path_csv = os.path.join(save_dir, "traffic_flows_east_processed.csv")
...
TIME_HEADWAY_DEL_THRESH = 0.1  # 车头时距0~0.1删除行
TTC_DEL_THRESH = 0.1  # TTC 0~0.1删除行
# 通用配置
QUANTILE = 0.999
VALID_RATIO_THRESH = 0.8  # 有效数据占比<80%则删除车辆
...
# ---------------------- 步骤2：碰撞时间重新核算（插入buquan代码逻辑） ----------------------
def recalculate_ttc_logic(df, trace_list):
    """
    重新核算TTC逻辑，参考buquan代码中的逻辑
    """
    df = df.sort_values(by=["ID", "Frame"]).reset_index(drop=True)
    print("\n=====步骤2核算 开始重新核算TTC及相关参数 =====")

    # TTC阈值参数
    TTC_LOW_THRESHOLD = 2  # TTC下限阈值
    TTC_HIGH_THRESHOLD = 100  # TTC上限阈值

    # 重新计算TTC < 阈值或TTC > 阈值的数据
    low_ttc_data = df[(df['TTC'] < TTC_LOW_THRESHOLD)].copy()
    high_ttc_data = df[(df['TTC'] > TTC_HIGH_THRESHOLD)].copy()
        ...
        for idx, row in combined_ttc_data.iterrows():
            ...
            if front_id != 0:  # 如果有前车
                ...
                if len(front_data) > 0:
                    ...
                    # 计算TTC（碰撞时间）
                    velocity_diff = current_velocity - front_velocity  # 当前车速减前车车速
                    ...
                    # 更新原数据
                    df.loc[idx, 'Following_dist'] = following_dist
                    df.loc[idx, 'Time_Headway'] = time_headway
                    df.loc[idx, 'TTC'] = ttc
                else:
                    # 如果找不到前车数据，设置TTC为0
                    df.loc[idx, 'Following_dist'] = 0.0
                    df.loc[idx, 'Time_Headway'] = 0.0
                    df.loc[idx, 'TTC'] = 0.0
            else:
                # 无前车，设置TTC为0
                df.loc[idx, 'Following_dist'] = 0.0
...
traffic_flows_west = recalculate_ttc_logic(traffic_flows_west, trace_records)
gc.collect()
...
def clean_xy_coords(df, trace_list):
    ...
    print(f"步骤3后行数：{len(df)}（删除坐标持续超出行：{len(delete_xy_rows)}，坐标漂移修改行：{len(drift_xy_rows)}）")
...
def clean_velocity(df, trace_list):
    ...
    print(f"步骤4后行数：{len(df)}（删除速度持续偏离行：{len(delete_vel_rows)}，速度漂移修改行：{len(drift_vel_rows)}）")
...
# ---------------------- 步骤5：Acceleration预处理（单调性检测优化版-修复索引问题） ----------------------
def clean_acceleration_with_monotonicity(df, trace_list):
    ...
    print(f"步骤5后行数：{len(df)}（删除加速度持续偏离行：{len(delete_acc_rows)}，加速度漂移修改行：{len(drift_acc_rows)}）")
...
def clean_vel_acc_components(df, trace_list):
    ...
    if all(col in df.columns for col in ["long_Vel", "lat_Vel"]):
        df["vec_velocity_mps"] = np.sqrt(df["long_Vel"] ** 2 + df["lat_Vel"] ** 2).round(2)
        df["vec_velocity_kmh"] = (df["vec_velocity_mps"] * 3.6).round(2)
        df["vel_vec_dev"] = abs(df["Velocity"] - df["vec_velocity_kmh"]).round(2)
    ...
    print(f"步骤6后行数：{len(df)}（速度/加速度分量处理完成）")
    return df
...
def clean_following_dist_with_monotonicity(df, trace_list):
    ...
    if not delete_dist_veh_rows.empty:
        ...
        delete_dist_veh_rows["modify_before"] = delete_dist_veh_rows.apply(
            lambda x: f"Following_dist={x[col_name]:.2f} m（有效占比<80%）", axis=1
        )
    ...
    df = df.drop(columns=aux_cols)
    print(f"步骤7后行数：{len(df)}（跟车距离处理完成）")
    return df
...
def clean_time_headway_with_monotonicity(df, trace_list):
    ...
    print(f"  - 删除0~0.1s车头时距行：{len(delete_thw_rows)}")
    ...
    if not delete_thw_veh_rows.empty:
        ...
        delete_thw_veh_rows["modify_before"] = delete_thw_veh_rows.apply(
            lambda x: f"Time_Headway={x[col_name]:.2f} s（有效占比<80%）", axis=1
        )
    ...
    aux_cols = [col for col in df.columns if col.startswith(f"{col_name}_")]
    df = df.drop(columns=aux_cols)
    print(f"步骤8后行数：{len(df)}（车头时距处理完成）")
    return df
...
def clean_ttc_with_monotonicity(df, trace_list):
    ...
    if not delete_ttc_veh_rows.empty:
        ...
        delete_ttc_veh_rows["modify_before"] = delete_ttc_veh_rows.apply(
            lambda x: f"TTC={x[col_name]:.2f} s（有效占比<80%）", axis=1
        )
...
# ---------------------- 步骤10：Velocity单位转换（km/h → m/s） ----------------------
print("\n📏 开始转换Velocity单位：km/h → m/s")
traffic_flows_west["Velocity"] = (traffic_flows_west["Velocity"] / 3.6).round(2)
print("✅ Velocity单位转换完成，km/h → m/s,已保留两位小数")
...
print(f"程序运行时间: {running_time:.6f} 秒")
# 执行步骤14：按新要求分析溯源记录

#检查每个车辆ID的数据连贯性，输出其需要补全的帧数
#根据连贯性插帧补全，之后重新核算碰撞时间
#读取traffic_flows_west/east_processed,输出traffic_flows_complete0数据集
import pandas as pd
import numpy as np
import gc
import os
...
# TTC阈值参数
TTC_LOW_THRESHOLD = 2  # TTC下限阈值
TTC_HIGH_THRESHOLD = 100  # TTC上限阈值
TTC_min = 0

# 打印行数参数
LOW_TTC_PRINT_ROWS = 10  # TTC < 阈值时打印的行数
HIGH_TTC_PRINT_ROWS = 10  # TTC > 阈值时打印的行数
SPECIAL_TTC_PRINT_ROWS = 10  # 特殊TTC变化（<2变为>100）时打印的行数
#数据读取西向
...
# traffic_flows_buquan_trace_path = os.path.join(save_dir, "traffic_flows_west_buquan_trace.csv")  # 补全数据记录

#数据读取东向
save_dir = r"E:\0little\read\CQSkyEyedata5\location5e"
west_pkl_processed_path = os.path.join(save_dir, "traffic_flows_east_processed.pkl")  # 待补全数据库
west_pkl_path = os.path.join(save_dir, "traffic_flows_east.pkl")  # 源数据库
traffic_flows_path = os.path.join(save_dir, "traffic_flows_complete0.csv")  # 补全数据汇集
traffic_pkl_flows_path = os.path.join(save_dir, "traffic_flows_complete0.pkl")  # 补全数据汇集
traffic_flows_buquan_trace_path = os.path.join(save_dir, "traffic_flows_east_buquan_trace.csv")  # 补全数据记录


print(f"{'=' * 50}")
print(f"数据处理开始: 交通流数据补全与TTC重新计算（增强版 - 使用源数据库进行TTC计算）")
print(f"TTC阈值设置: < {TTC_LOW_THRESHOLD} 或 > {TTC_HIGH_THRESHOLD}")
print(f"打印行数设置: TTC < 阈值打印{LOW_TTC_PRINT_ROWS}行, TTC > 阈值打印{HIGH_TTC_PRINT_ROWS}行")
print(f"特殊TTC变化打印{SPECIAL_TTC_PRINT_ROWS}行")
...
print(f"补全数据记录CSV路径: {traffic_flows_buquan_trace_path}")
print(f"{'=' * 50}\n")
...
traffic_flows_west_processed = pd.read_pickle(west_pkl_processed_path)  # 待补全数据库1
    ...
    for vehicle_id in all_vehicle_ids:
        ...
        for gap_start, gap_end, missing_frames in time_gaps:
            ...
            for k in range(1, missing_frames + 1):
                ...
                # 记录补全数据（包含补全类型和所有字段信息）
                buquan_record = filled_row.copy()
                buquan_record['Buquan_Type'] = 'buquan0'
                buquan_record['Reliability'] = 'High' if is_high_reliability else 'Low'
                buquan_trace_data.append(buquan_record)

        print(f"  车辆ID {vehicle_id} 补全完成，补全了 {sum([gap[2] for gap in time_gaps]):,} 帧")

    return pd.DataFrame(filled_data), pd.DataFrame(buquan_trace_data)

# 执行buquan0的补全
print("\n开始执行buquan0补全逻辑（参考源数据库）...")
filled_df_0, buquan_trace_0 = fill_missing_frames()
...
for info in missing_frame_info_1:
    ...
    for k in range(1, missing_count + 1):
        ...
        # 记录buquan1补全数据
        buquan_record = new_row.copy()
        buquan_record['Buquan_Type'] = 'buquan1'
        buquan_record['Reliability'] = 'High' if is_high_reliability else 'Low'
        buquan_trace_1.append(buquan_record)
...
if filled_data_1:
    ...
    filled_df_1 = pd.DataFrame()  # 创建空DataFrame

# 将buquan_trace_1转换为DataFrame，即使没有补全数据
buquan_trace_1 = pd.DataFrame(buquan_trace_1) if buquan_trace_1 else pd.DataFrame()
...
# 5. TTC核算处理前后的统计对比
# ===============================
print(f"开始进行TTC核算处理前后的统计对比...")

# 统计TTC核算处理前的数据分布
print(f"\nTTC核算处理前的数据分布:")
before_ttc_0 = len(traffic_flows_west_complete[traffic_flows_west_complete['TTC'] == TTC_min])
before_ttc_0_to_2 = len(traffic_flows_west_complete[(traffic_flows_west_complete['TTC'] > TTC_min) & (traffic_flows_west_complete['TTC'] < TTC_LOW_THRESHOLD)])
before_ttc_gt_100 = len(traffic_flows_west_complete[traffic_flows_west_complete['TTC'] > TTC_HIGH_THRESHOLD])

print(f"  TTC = 0: {before_ttc_0:,} 行")
print(f"  0 < TTC < {TTC_LOW_THRESHOLD}: {before_ttc_0_to_2:,} 行")
print(f"  TTC > {TTC_HIGH_THRESHOLD}: {before_ttc_gt_100:,} 行")

# 保存原始TTC数据用于对比分析
original_ttc_data = traffic_flows_west_complete[['ID', 'Frame', 'TTC']].copy()
original_ttc_data.columns = ['ID', 'Frame', 'Original_TTC']

# 保存原始TTC数据用于后续分析
traffic_flows_west_complete_with_original = traffic_flows_west_complete.copy()
low_ttc_data = traffic_flows_west_complete[(traffic_flows_west_complete['TTC'] < TTC_LOW_THRESHOLD)].copy()
#low_ttc_data = traffic_flows_west_complete[(traffic_flows_west_complete['TTC'] > TTC_min)& (traffic_flows_west_complete['TTC'] < TTC_LOW_THRESHOLD)].copy()
high_ttc_data = traffic_flows_west_complete[traffic_flows_west_complete['TTC'] > TTC_HIGH_THRESHOLD].copy()

print(f"发现 {len(low_ttc_data):,} 行TTC < {TTC_LOW_THRESHOLD}的数据")
print(f"发现 {len(high_ttc_data):,} 行TTC > {TTC_HIGH_THRESHOLD}的数据")

# 记录原始TTC<2的数据
original_low_ttc_data = low_ttc_data.copy()

# 合并需要重新计算的TTC数据
combined_ttc_data = pd.concat([low_ttc_data, high_ttc_data]).drop_duplicates()
if len(combined_ttc_data) > 0:
    print(f"总共需要重新计算TTC的数据: {len(combined_ttc_data):,} 行")
    print("正在重新计算TTC...")
    # 为每个TTC < 阈值或TTC > 阈值的数据重新计算
    for idx, row in combined_ttc_data.iterrows():
        ...
        if front_id != 0:  # 如果有前车
            # 在源数据库中查找前车数据（在相同时间点）
            front_data = traffic_flows_west[
                (traffic_flows_west['ID'] == front_id) &
                (abs(traffic_flows_west['time'] - current_time) < 0.001)  # 使用时间精度匹配
                ]
            ...
            if len(front_data) > 0:
                ...
                # 计算跟车距离
                following_dist = round(abs(front_x - current_x), 2)
...
# 统计TTC核算处理后的数据分布
print(f"\nTTC核算处理后的数据分布:")
after_ttc_0 = len(traffic_flows_west_complete[traffic_flows_west_complete['TTC'] == TTC_min])
after_ttc_0_to_2 = len(traffic_flows_west_complete[(traffic_flows_west_complete['TTC'] > TTC_min) & (traffic_flows_west_complete['TTC'] < TTC_LOW_THRESHOLD)])
after_ttc_gt_100 = len(traffic_flows_west_complete[traffic_flows_west_complete['TTC'] > TTC_HIGH_THRESHOLD])

print(f"  TTC = {TTC_min}: {after_ttc_0:,} 行")
print(f"  {TTC_min} < TTC < {TTC_LOW_THRESHOLD}: {after_ttc_0_to_2:,} 行")
print(f"  TTC > {TTC_HIGH_THRESHOLD}: {after_ttc_gt_100:,} 行")

# 显示TTC变化对比
print(f"\nTTC变化对比:")
print(f"  TTC = 0: 从 {before_ttc_0:,} 行变为 {after_ttc_0:,} 行 (变化: {after_ttc_0 - before_ttc_0:+,})")
print(f"  0 < TTC < {TTC_LOW_THRESHOLD}: 从 {before_ttc_0_to_2:,} 行变为 {after_ttc_0_to_2:,} 行 (变化: {after_ttc_0_to_2 - before_ttc_0_to_2:+,})")
print(f"  TTC > {TTC_HIGH_THRESHOLD}: 从 {before_ttc_gt_100:,} 行变为 {after_ttc_gt_100:,} 行 (变化: {after_ttc_gt_100 - before_ttc_gt_100:+,})")

# 再次检查TTC < 阈值且TTC != 0的数据
print(f"\n重新计算后，检查TTC < {TTC_LOW_THRESHOLD}的数据...")
final_low_ttc_data = traffic_flows_west_complete[
  (traffic_flows_west_complete['TTC'] < TTC_LOW_THRESHOLD) & (traffic_flows_west_complete['TTC'] != 0)].copy()
#final_low_ttc_data = traffic_flows_west_complete[traffic_flows_west_complete['TTC'] < TTC_LOW_THRESHOLD].copy()


print(f"重新计算后仍有 {len(final_low_ttc_data):,} 行TTC < {TTC_LOW_THRESHOLD}且TTC != 0的数据")

# 检查TTC > 阈值的数据
print(f"\n重新计算后，检查TTC > {TTC_HIGH_THRESHOLD}的数据...")
final_high_ttc_data = traffic_flows_west_complete[
    (traffic_flows_west_complete['TTC'] > TTC_HIGH_THRESHOLD)].copy()
print(f"重新计算后仍有 {len(final_high_ttc_data):,} 行TTC > {TTC_HIGH_THRESHOLD}的数据")

# 分析特殊TTC变化：从TTC < 阈值变为TTC > 阈值的数据
print(f"\n分析特殊TTC变化：从TTC < {TTC_LOW_THRESHOLD}变为TTC > {TTC_HIGH_THRESHOLD}的数据...")

# 重新合并原始TTC数据
traffic_flows_west_complete_with_original = (
    traffic_flows_west_complete.merge(original_ttc_data, on=['ID', 'Frame'], how='left'))

# 找出从TTC < 阈值变为TTC > 阈值的数据
special_ttc_changes = traffic_flows_west_complete_with_original[
    (traffic_flows_west_complete_with_original['Original_TTC'] <= TTC_min ) &
    (traffic_flows_west_complete_with_original['TTC'] > TTC_HIGH_THRESHOLD)
].copy()

print(f"发现 {len(special_ttc_changes):,} 行从TTC =< {TTC_min} (均为TTC=0)变为TTC > {TTC_HIGH_THRESHOLD}的数据")

#打印TTC特殊变化的数据行数
if len(special_ttc_changes) > 0:
    print(f"\n特殊TTC变化详细信息（前{min(SPECIAL_TTC_PRINT_ROWS, len(special_ttc_changes))}行）：")
    for idx, row in special_ttc_changes.head(SPECIAL_TTC_PRINT_ROWS).iterrows():
        current_id = int(row['ID'])
        front_id = int(row['FrontID'])
        following_dist = row['Following_dist']
        current_velocity = row['Velocity']
        original_ttc = row['Original_TTC']
        new_ttc = row['TTC']

        # 获取前车速度
        front_data = traffic_flows_west[
            (traffic_flows_west['ID'] == front_id) &
            (abs(traffic_flows_west['time'] - row['time']) < 0.001)
            ]
        ...
        print(
            ...
            f"前车速度: {front_velocity:.2f}, 原TTC: {original_ttc:.2f}, 新TTC: {new_ttc:.2f}")

#打印依旧关于TTC小于阈值且不等于0的行数
if len(final_low_ttc_data) > 0:
    print(f"\nTTC < {TTC_LOW_THRESHOLD}且TTC != 0的详细信息"
          f"（前{min(LOW_TTC_PRINT_ROWS, len(final_low_ttc_data))}行）：")
    for idx, row in final_low_ttc_data.head(LOW_TTC_PRINT_ROWS).iterrows():
        current_id = int(row['ID'])
        front_id = int(row['FrontID'])
        following_dist = row['Following_dist']
        current_velocity = row['Velocity']
        ...
        print(
            f" 帧：{int(row['Frame'])}, 车辆ID: {current_id}, 前车ID: {front_id}, 跟车距离: {following_dist:.2f}, "
            f"当前车速: {current_velocity:.2f}, 前车速度: {front_velocity:.2f}, TTC: {ttc:.2f}")

#打印TTC依旧大于阈值的行数包括经过修改变成大于阈值的行数
if len(final_high_ttc_data) > 0:
    print(f"\nTTC > {TTC_HIGH_THRESHOLD}的详细信息（前{min(HIGH_TTC_PRINT_ROWS, len(final_high_ttc_data))}行）：")
    for idx, row in final_high_ttc_data.head(HIGH_TTC_PRINT_ROWS).iterrows():
        current_id = int(row['ID'])
        front_id = int(row['FrontID'])
        following_dist = row['Following_dist']
        current_velocity = row['Velocity']
...
for vehicle_id in all_vehicle_ids_complete:
    # 筛选特定车辆ID的数据
    vehicle_data = traffic_flows_west_complete_with_original[traffic_flows_west_complete_with_original['ID'] == vehicle_id].copy()
...
# ===============================
# print("\n开始删除特定ID的数据行...")
# ids_to_remove = [71, 341, 3206, 3705, 5818, 5961, 6185]
# print(f"需要删除的ID列表: {ids_to_remove}")
#
# # 统计删除前的数据量
# original_count = len(traffic_flows_west_complete_with_original)
# print(f"删除前数据量: {original_count:,} 行")
#
# # 删除指定ID的数据行
# traffic_flows_west_complete_with_original = traffic_flows_west_complete_with_original[
#     ~traffic_flows_west_complete_with_original['ID'].isin(ids_to_remove)
# ]
#
# # 统计删除后的数据量
# remaining_count = len(traffic_flows_west_complete_with_original)
# print(f"删除后数据量: {remaining_count:,} 行")
# print(f"共删除了 {original_count - remaining_count:,} 行数据")
...
numeric_columns = ['time', 'X', 'Y', 'Length', 'Width', 'Velocity', 'Acceleration', 'long_Vel', 'lat_Vel', 'long_Acc',
                   ...
                   'Following_dist', 'Time_Headway', 'TTC', 'Original_TTC']
for col in numeric_columns:
    if col in traffic_flows_west_complete_with_original.columns:
        traffic_flows_west_complete_with_original[col] = traffic_flows_west_complete_with_original[col].round(2)

# 合并补全数据记录
if not buquan_trace_0.empty and not buquan_trace_1.empty:
    all_buquan_trace_data = pd.concat([buquan_trace_0, buquan_trace_1], ignore_index=True)
elif not buquan_trace_0.empty:
    all_buquan_trace_data = buquan_trace_0
elif not buquan_trace_1.empty:
    all_buquan_trace_data = buquan_trace_1
else:
    all_buquan_trace_data = pd.DataFrame()  # 创建空DataFrame

# 保存补全后的数据
print(f"\n正在保存补全后的数据到 {traffic_flows_path}...")
traffic_flows_west_complete_with_original.to_csv(traffic_flows_path, index=False, encoding='utf-8-sig')
traffic_flows_west_complete_with_original.to_pickle(traffic_pkl_flows_path)
print(f"数据已保存到: {traffic_flows_path} 和 {traffic_pkl_flows_path}")

# 保存补全数据记录
print(f"\n正在保存补全数据记录到 {traffic_flows_buquan_trace_path}...")
all_buquan_trace_data.to_csv(traffic_flows_buquan_trace_path, index=False, encoding='utf-8-sig')
print(f"补全数据记录已保存到: {traffic_flows_buquan_trace_path}")

# 最终信息
print(f"\n{'=' * 50}")
print(f"数据补全、TTC重新计算、连续性验证和数据删除完成！")
print(f"最终数据量: {len(traffic_flows_west_complete_with_original):,} 行")
print(f"buquan0补全数据量: {len(filled_df_0):,} 行")
print(f"buquan1补全数据量: {len(filled_df_1):,} 行")
print(f"总补全数据量: {len(filled_df_0) + len(filled_df_1):,} 行")
print(f"补全数据记录量: {len(all_buquan_trace_data):,} 行")
print(f"TTC < {TTC_LOW_THRESHOLD}且TTC != 0的数据量: {len(final_low_ttc_data):,} 行")
print(f"TTC > {TTC_HIGH_THRESHOLD}的数据量: {len(final_high_ttc_data):,} 行")
print(f"从TTC < {TTC_LOW_THRESHOLD}变为TTC > {TTC_HIGH_THRESHOLD}的数据量: {len(special_ttc_changes):,} 行")
# print(f"删除的ID数量: {len(ids_to_remove)} 个")
# print(f"删除的数据行数: {original_count - remaining_count:,} 行")
print(f"{'=' * 50}\n")

# 清理内存
del traffic_flows_west, traffic_flows_west_processed, all_vehicle_ids, all_vehicle_ids_complete, filled_df_0, filled_df_1, traffic_flows_west_complete, traffic_flows_west_complete_with_original
gc.collect()
print("内存已清理完成")